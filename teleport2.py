...
def teleport(x,y,z,distance = 13):
    Misc.SendMessage("direcao" + Player.Direction,14) 
    if Player.Direction == "North" or Player.Direction == "South" or Player.Direction == "Up" or Player.Direction == "Right" or Player.Direction == "Left": 
        distance = 11
    ...
    if wait_for_target("Teleport",1300):
        Target.TargetExecuteRelative(Player.Serial,distance)
        Misc.Pause(10)
    return
...
if Items.BackpackCount(0x1F42, 0) > 0: # TP Scrolls
    ...
    teleport(Player.Position.X,Player.Position.Y,Player.Position.Z, 11)  
Misc.Pause(10)    
