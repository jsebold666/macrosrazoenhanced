...
def HelpFriend():
    ...
    if mob_para is not None:
        if mob_para.Paralized and Timer.Check("feeble") == False:
            ...
            Target.TargetExecute(mob_para.Serial)
            Misc.Pause(30)
            Timer.Create("feeble", 200)
            return
    mob = Target.GetTargetFromList("friends")
    Misc.Pause(30)
    if mob is not None:
        ...
        if not mob.YellowHits:
            ...
            if Player.GetSkillValue('Chivalry') > 30 and not guardszone:
                Spells.CastChivalry("Remove Curse")
                Target.WaitForTarget(2000,True)
                Target.TargetExecute(mob.Serial)
                Misc.Pause(50)
                return
        if Timer.Check("help") == False:
            Player.HeadMessage(13,"None to help")
            Timer.Create("help", 500)
