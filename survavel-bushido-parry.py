...
# --------------------------- STATUS ----------------------------------
# ---------------------------------------------------------------------
status_str_mais_um = 147
status_dex_mais_um = 65
...
def MultiTrapPouchUse():
    ...
    with open(str(Misc.CurrentScriptDirectory())+'\\'+Player.Name+"-pouches.txt", 'w') as pouches_file:
        ...
        if not used and Timer.Check("pouches") == False:
            Player.HeadMessage(55, "[DANGER] NO POUCHES TRAPPED")
    ...
    while True:
        ...
        if not Timer.Check("timer_warning"):
            ...
            if refresh == 0:
                ...
                if refresh < 10:
                    Misc.SendMessage("[Alert] Refresh pots!")
                    Misc.Pause(10)
                elif refresh < 20:
                    Player.HeadMessage( 32, "[Warning] Refresh pots!")     
                    Misc.Pause(10)
            ...
            if CountTrapPouchUse() > 0:
                Misc.SendMessage( "POUCH TRAPPED IS " + str(CountTrapPouchUse()), 32)
                Misc.Pause(10)
            if not auto_chug_pots:
                Misc.SendMessage("Unbalanced, autopotions off", 32)
                Misc.Pause(10)
            if Player.Str < 158 and not Target.HasTarget():
                Player.HeadMessage(32, "STR Low")
                Misc.Pause(10)
                
            if Player.Dex < 70 and not Target.HasTarget():
                Player.HeadMessage(32, "Dex Low")
                Misc.Pause(10)
        # ---------------------------------------------------------------------
        # --------------------------- APPLE -----------------------------------
        # ---------------------------------------------------------------------      
        ...
        # ---------------------------------------------------------------------
        if Journal.Search("The attack has temporarily paralyzed you!") and Player.Hits > 70:
