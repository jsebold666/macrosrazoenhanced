...
if __name__ == '__main__':
    while not Player.IsGhost: 
        ...
        if "Esnaor Senju" in Player.Name and Player.GetItemOnLayer("RightHand") is None and Player.GetItemOnLayer("LeftHand") is None and Timer.Check("disarmed") == False:
        ...
        if mob is not None:
            if Player.DistanceTo(mob) < 16:
                ...
                if Player.Dex < status_dex and not Target.HasTarget():
                    ...
                    Misc.Pause(600)
            if Player.DistanceTo(mob) < 11:
                if mob.Paralized:
                    Player.HeadMessage( 62, "CONFLAG NO FDP!!")
        ...
        # --------------------------- REFRESH ---------------------------------
        # ---------------------------------------------------------------------
        if Player.Stam < Player.StamMax*0.7 and not Target.HasTarget():
            refresh = Items.BackpackCount(0xf0b, -1) 
            if(refresh > 0): 
        ...
        if Player.BuffsExist("Evil Omen"):
            ...
            Player.HeadMessage( 62, "Evil Omen - Posion pot!!")
            Misc.Pause(600)
